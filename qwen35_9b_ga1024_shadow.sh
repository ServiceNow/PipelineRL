...
# --config-name terminal_ga1024_shadow (shadow probe credit + probe-suffixed
# fleet DNS so it never clashes with the mainline run's fleets).

TIMESTAMP=$(date +%s)
OUTPUT_DIR_BASE=/mnt/llmd/results/exps/rafa/terminal
JOB_NAME=${JOB_NAME:-terminal_qwen35_9b_ga1024_shadow_01}
...
--config-name terminal_ga1024_shadow \
