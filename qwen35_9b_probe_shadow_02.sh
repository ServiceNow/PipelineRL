#!/bin/bash
# Frozen-probe SHADOW validation run, parallel to the gspo_N mainline.
# Same 4 nodes x 8 H100 recipe as qwen35_9b_terminal.sh but with
# --config-name terminal_probe_shadow_02 (shadow probe credit + probe-suffixed
# fleet DNS so it never clashes with the mainline run's fleets).

TIMESTAMP=$(date +%s)
OUTPUT_DIR_BASE=/mnt/llmd/results/exps/rafa/terminal
JOB_NAME=${JOB_NAME:-terminal_qwen35_9b_probe_shadow_02}
...
--config-name terminal_probe_shadow_02 \
