...
# TODO: Rename to each dataset
def process_coding_cpp(dataset, dataset_name):
    for sample in dataset:
        unit_tests = json.loads(sample['reward_model']['ground_truth'])
        yield {
            "dataset": dataset_name,
            "task": sample["prompt"][0]['content'],
            "language": sample['extra_info']['language'],
            "test_inputs": unit_tests["inputs"],
            "test_expected_outputs": unit_tests["outputs"],
            "fn_name": unit_tests["fn_name"],
        }
...
def load_datasets(dataset_names: List[str] | str | None, seed: int | None = None) -> List[Tuple[str, Dict]]:
    ...
    if "coding_cpp" in dataset_names:
        dataset = load_dataset(
            "ServiceNow-AI/coding-rlvr-dataset", 
            "verifiable_coding_problems_cpp",
            split="train",
            trust_remote_code=True)
        
        dataset = dataset.shuffle(seed=seed) if seed is not None else dataset
        samples = [s for s in process_coding_cpp(dataset, "verifiable_coding_problems_cpp") if s is not None]
        logger.info(f"Loading ServiceNow-AI/coding-rlvr-dataset train dataset: {len(samples)} samples")
