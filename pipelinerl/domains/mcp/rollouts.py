...
from pipelinerl.utils import (
    get_environment_jobs,
    resolve_environment_key,
    select_environment_config,
)
...
async def generate_mcp_rollout(
    ...
    env_key = resolve_environment_key(cfg, default="mcp")
    ...
    if cfg.world.environment_mode == "remote":
        ...
        env_cfg = select_environment_config(cfg, key=env_key)
        if env_cfg is None:
            raise RuntimeError("No embedded environment configuration available")
        env_worker = _get_embedded_worker(env_cfg, concurrency)
