...
class WorldMap:
    def __init__(self, cfg: DictConfig, verbose: bool = False):
        ...
        self.available_gpus: dict[int, set] = {
            i: set(range(self.node_size)) for i in reversed(range(self.world_size))
        }
        ...
        for node, remaining_gpus in self.available_gpus.items():
            ...
            if gpus:
                self.job_map[node].append(
                    Job(kind="finetune", replica_idx=node, node_rank=node, gpus=gpus)
                )
        ...
        for node, jobs in self.job_map.items():
            ...
            for job in jobs:
                self._log_info(
                    f"  {job.kind} {job.replica_idx} on gpus {job.gpus}, local idx {job.local_idx}"
                )

    def _split_gpus_by_purpose(self, cfg):
        fraction_sum = (
            cfg.world.actor_fraction
            + cfg.world.preprocessor_fraction
            + cfg.world.finetune_fraction
        )
        ...
        desired_actor_gpu_share = max(
            int(total_gpus * actor_fraction), self.gpus_per_llm
        )
        ...
        desired_finetune_gpu_share = (
            total_gpus - desired_actor_gpu_share - desired_preprocessor_gpu_share
        )
        self._log_info(
        ...
        gpus_per_actor = (
            int(desired_actor_gpu_share / cfg.world.actors)
            if cfg.world.actors > 0
            else 0
        )
        ...
        gpus_per_preprocessor = (
            ...
            if cfg.world.preprocessors > 0
            else 0
        )
        gpus_per_preprocessor = gpus_per_preprocessor - (
            gpus_per_preprocessor % self.gpus_per_llm
        )
        self.llms_per_actor = (
            max(int(gpus_per_actor / self.gpus_per_llm), 1) if gpus_per_actor > 0 else 0
        )
        self.total_actor_llms = self.llms_per_actor * cfg.world.actors
        self.llms_per_preprocessor = (
            max(int(gpus_per_preprocessor / self.gpus_per_llm), 1)
            if gpus_per_preprocessor > 0
            else 0
        )
        ...
        self.total_finetune_gpus = (
            total_gpus - total_actor_gpus - total_preprocessor_gpus
        )
        self._log_info(
        ...
        self._log_info(
            "I have adjusted the GPU shares to accomodate these constraints."
        )
        self._log_info(
        ...
        if self.total_finetune_gpus == 0:
            logger.warning(
                "No GPUs left for finetune workers. You can still debug other parts of the pipeline."
            )
        ...
        for worker_idx in range(cfg.world.actors):
            for actor_llm_idx in range(self.llms_per_actor):
                node = next(
                    (
                        node
                        for node in self.available_gpus
                        if len(self.available_gpus[node]) >= self.gpus_per_llm
                    ),
                    None,
                )
                if node is None:
                ...
                if not actor_placed:
                    ...
                    actor_placed = True
                gpus = [
                    self.available_gpus[node].pop() for _ in range(self.gpus_per_llm)
                ]
                local_idx = min(gpus)
    ...
    def nodes_with_finetuning(self) -> list[int]:
        return [
            node
            for node, jobs in self.job_map.items()
            if any(job.kind == "finetune" for job in jobs)
        ]
