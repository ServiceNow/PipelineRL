...
def rl_step(
    ...
    if frozen_probe_turn_probs is not None:
        ...
        # micro-batches hold a handful of turns each (often single-class), so any
        # AUROC computed here is quantized noise. The step-level aggregation in
        # finetune_loop pools pairs across micro-batches and ranks and derives
        # every frozen_probe/* metric from the union.
        stats["frozen_probe/pair_probs"] = frozen_probe_turn_probs.detach().cpu().tolist()
        stats["frozen_probe/pair_targets"] = frozen_probe_turn_targets.detach().cpu().tolist()
