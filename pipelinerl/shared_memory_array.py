...
class EntrySizeExceeded(ValueError):
    """Raised when a serialized object is larger than the shared-memory entry cap.

    Subclasses ValueError so existing callers that catch ValueError keep working,
    while callers that want to drop-not-crash on oversize can match this type
    specifically and still let unrelated errors propagate.
    """
...
class SharedMemoryQueue:
    ...
    def put(self, item: Any, block: bool = True, timeout: float | None = None) -> None:
        ...
        # Store the item in the shared array. The slot is reserved before
        # serialization, so a failed write (e.g. EntrySizeExceeded) must return
        # the slot; otherwise it leaks and a small queue soon deadlocks.
        try:
