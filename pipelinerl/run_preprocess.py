...
def replace_oov_tokens_with_the(
    ...
    for entry in data:
        ...
        if invalid_token_ids:
            patched_entries += 1
            logger.warning(
                f"Patching entry with invalid token ids: {invalid_token_ids}"
            )
            ...
            if "logprobs" in entry and len(entry["logprobs"]) > 0:
                ...
                for i, token_id in enumerate(invalid_token_ids):
                    if i + completion_start < len(entry["input_ids"]):
                        logger.warning(
                            f"Invalid token in completion part, logprobs may be inconsistent"
                        )
    ...
    if patched_entries > 0:
        logger.warning(
            f"Patched {patched_entries} entries with invalid token ids from {len(data)}"
        )
...
def preprocess_dataset(
    ...
    dataset = populate_rl_data(
        dataset=dataset, eos_token_id=tokenizer.eos_token_id, config=rl_config
    )
    dataset = dataset.add_column(
        "model_version", [entry["metadata"]["model_version"] for entry in data]
    )  # type: ignore
                    ...
                    if pop_old_data:
                        try:
                            ...
                            if old_and_dropped // 100 != last_time_notice:
                                logger.info(
                                    f"So far removed {old_and_dropped} old elements from preprocessor queue"
                                )
...
def run_preprocessing_loop(
    ...
    stats_aggregator = SlidingWindowAggregator(
        window_size=max(10, 1000 // cfg.preprocess.chunk_size)
    )

    buffer = []
    with (
        write_to_streams(output_stream) as writer,
        write_to_streams(stats_streams) as stats_writer,
    ):
        with mp.Manager() as manager, SharedMemoryManager() as smm:
            ...
            logger.info(
                f"Shared memory buffer size: {io_buffer.get_memory_size() / 2**30} Gb"
            )
                    ...
                    try:
                        ...
                        if submitted_chunks - processed_chunks < max_pool_tasks:
                            try:
                                ...
                                submitted_chunks += 1
                                next_llm_index = (
                                    (next_llm_index + 1) % len(llms) if llms else 0
                                )
                            except Empty:
                                pass
                        ...
                        if cfg.preprocess.buffer_size:
                            ...
                            logger.info(
                                f"Buffer is full with {len(buffer)} samples, start writing"
                            )
                        ...
                        stats_aggregator.update(
                            [len(entry["input_ids"]) for entry in buffer]
                        )
                        published_samples += len(buffer)
                        max_model_version = max(
                            [dataset["model_version"] for dataset in buffer]
                        )
                        samples_in_queue = (
                            dataset_queue.qsize() * cfg.preprocess.chunk_size
                        )
                        stats = {
                            "preprocessor/published_samples": published_samples,
                            "preprocessor/published_model_version": max_model_version,
                            "preprocessor/samples_in_input_queue": raw_chunk_queue.qsize()
                            * cfg.preprocess.chunk_size,
                        ...
                        if stats_aggregator.has_enough_data():
                            stats.update(
                                {
                                    "preprocessor/" + k: v
                                    for k, v in stats_aggregator.get_stats().items()
                                }
                            )
                        run.log(stats)
